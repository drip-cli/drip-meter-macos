...
public enum UsageReport {
    ...
    public static func markdown(_ input: Input) -> String {
        ...
        lines.append("| Dollars saved | \(formatDollars(r.dollarsSaved)) (@ $\(String(format: "%.2f", input.pricePerMtok))/Mtok) |")
        ...
        if let compaction = r.compaction, compaction.totalCompactions > 0 {
            lines.append("| Compactions | \(formatInt(compaction.totalCompactions)) (re-sent \(formatInt(compaction.tokensResentAfterCompaction)) tok) |")
        }
        lines.append("")
        ...
        // 5. Per-agent
        let activeAgents = input.agents.filter { $0.hasActivity }
        if !activeAgents.isEmpty {
            ...
            for agent in activeAgents {
                lines.append("| \(agent.agent.displayName) | \(agent.sessions) | \(agent.filesTracked) | \(formatInt(agent.tokensSaved)) | \(agent.reductionPct) % |")
            }
            lines.append("")
        }
        ...
        if !top.isEmpty {
            ...
            for file in top {
                ...
                lines.append("| `\(display)` | \(file.reads) | \(formatInt(file.tokensSaved)) | \(file.reductionPct) % |")
            }
            lines.append("")
        }
        ...
        lines.append("---")
        lines.append("")
        lines.append("Generated by [DripMeter](https://github.com/drip-cli/drip-meter-macos), the menu bar companion for [DRIP](https://github.com/drip-cli/drip).")
    ...
    private static func appendTable(_ lines: inout [String], buckets: [MeterReport.DayBucket]) {
        ...
        for bucket in buckets.reversed() {
            lines.append("| \(bucket.day) | \(bucket.reads) | \(formatInt(bucket.tokensSaved)) | \(bucket.reductionPct) % |")
        }
        lines.append("")
    }
